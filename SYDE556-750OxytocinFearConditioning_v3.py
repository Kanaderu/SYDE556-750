...
def US_function(t):
    if 0<t<1: return 0
    if 1<=t<=2: return 1 #training phase
    if 2<t<3: return 0
    return 0

def CS_function(t):
    if 1<t<2: return 1 #training phase
    if 3<t<4: return 1 #testing phase
    if 5<t<6: return 1 #oxytocin (TGOT) application phase
    if 7<t<8: return 1 #GABA (muscimol) application phase
    return 0
...
def GABA_function(t):
    if 7<t<8: return 1 #oxytocin application phase
    return 0
...
with model:
	...
	nengo.Connection(CeL_ON,CeL_OFF,transform=-1,synapse=tau_GABA)
