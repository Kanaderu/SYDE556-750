...
filename='FearConditioningMullerCombinedV4pt1'
...
#inhibitory interneuron connections out of LA_inter onto LA; temporary until Perisien
def LA_recurrent(x):
	cs=x[:dim] 
	feedback=cs*LA_inter_feedback
	return feedback

#inhibitory interneuron connections out of BA_inter onto BA (through IL/PL/CCK/PV)
def BS_recurrent_F_to_F(x):
	fear=x[2*dim:3*dim]
	extinct=x[3*dim:4*dim]
	feedback=fear*BA_inter_feedback_F_to_F #mutual excitation, won't work with gaba increasing activity
	return feedback

def BS_recurrent_E_to_E(x):
	fear=x[2*dim:3*dim]
	extinct=x[3*dim:4*dim]
	feedback=extinct*BA_inter_feedback_E_to_E #mutual excitation, won't work with gaba increasing activity
	return feedback
	
def BS_recurrent_F_to_E(x):
	fear=x[2*dim:3*dim]
	extinct=x[3*dim:4*dim]
	feedback=fear*BA_inter_feedback_F_to_E #mutual inhibition, works with gaba increasing activity
	return feedback
	
def BS_recurrent_E_to_F(x):
	fear=x[2*dim:3*dim]
	extinct=x[3*dim:4*dim]
	feedback=extinct*BA_inter_feedback_E_to_F #mutual inhibition, works with gaba increasing activity
	return feedback
...
def LA_inter_error(x):
    cs=x[:dim]
    us=x[dim:]
    error=us-cs #positivity of error population handles negative numbers appropriately
    return error
    
#difference between US and BA activity is used to train Context-BA_fear connection w/o extinction 
#and Context-BA_extinct connection w/ extinction 
def BA_inter_error(x):
    context=x[:dim]
    us=x[dim:2*dim]
    error=us-context #positivity/negativity of error populations handles negative numbers appropriately
    return error
...
with model:
	...
	LA_recurrent=nengo.Connection(LA,LA,transform=LA_inter_feedback)
	...
	#Representation: [context,US,Fear_recurrent,Extinct_recurrent]
	#Functions:
	...
	BA_inter=nengo.Ensemble(16*N,4*dim,radius=4)
	...
	nengo.Connection(stim_gaba,LA_inter.neurons,synapse=tau_stim,
	        transform=np.ones((4*N,1))) #stimulate the gabaergic interneurons directly
	nengo.Connection(stim_oxy,CeL_OFF.neurons,synapse=tau_stim,
	        transform=np.ones((N,1))) #stimulate the gabaergic interneurons directly
	nengo.Connection(stim_gaba,BA_inter.neurons,synapse=tau_stim,
	        transform=np.ones((16*N,1))) #stimulate the gabaergic interneurons directly
	
	#Lateral Amygdala connections
	conn_condition=nengo.Connection(C,LA,synapse=tau_learn,transform=0) #primary learned connection CS-freeze
	nengo.Connection(U,LA_inter[dim:],synapse=tau) #send US info to LA_inter, to compute error
	nengo.Connection(LA,LA_inter[:dim],synapse=tau_recurrent) #send CS representation to LA_inter, to compute error/feedback
	nengo.Connection(LA_inter,error_cond,synapse=tau_recurrent,function=LA_inter_error)
	#PERISIEN TRANSFORM HERE - strictly inhibit the LA population with negative weights (onto neurons?)
	nengo.Connection(LA_inter,LA.neurons,synapse=tau_recurrent,
	        transform=np.ones((N,1)), function=LA_recurrent) #recurrent inhibition from interneurons
            
    #Basal Nuclei connections, includes possible Cortex/Hippocampus connections
	nengo.Connection(LA,BA_fear,synapse=tau) #CS-fear circuit
	...
	nengo.Connection(BA_fear,BA_inter[2*dim:3*dim],synapse=tau) #corresponds to known LA to CCK connection
	nengo.Connection(BA_extinct,BA_inter[3*dim:4*dim],synapse=tau) #unknown
	#PERISIEN TRANSFORM HERE
	nengo.Connection(BA_inter,BA_fear.neurons,synapse=tau_recurrent,
	        transform=np.ones((N,1)),function=BS_recurrent_F_to_F) #IL
	nengo.Connection(BA_inter,BA_extinct.neurons,synapse=tau_recurrent,
	        transform=np.ones((N,1)),function=BS_recurrent_E_to_E) #???
	nengo.Connection(BA_inter,BA_extinct.neurons,synapse=tau_recurrent,
	        transform=np.ones((N,1)),function=BS_recurrent_F_to_E) #CCK
	nengo.Connection(BA_inter,BA_fear.neurons,synapse=tau_recurrent,
	        transform=np.ones((N,1)),function=BS_recurrent_E_to_F) #PV
