...
with model:
	...
	U=nengo.Ensemble(N,dim,radius=2) #excited by stim_US
	C=nengo.Ensemble(N,dim) #excited by stim_CS
	Context=nengo.Ensemble(N,dim) #excited by stim_context
	...
	conn_condition=nengo.Connection(C,LA[:dim],synapse=tau_stim,function=lambda x: [0]*dim) #primary learned connection CS-freeze
	nengo.Connection(LA,error_cond,synapse=tau,function=LA_error_cond) #send CS and US rep  to error
	...
	conn_context=nengo.Connection(Context,BA_fear[:dim],synapse=tau_stim,function=lambda x: [0]*dim) #context-fear circuit
	conn_extinct=nengo.Connection(Context,BA_extinct[:dim],synapse=tau_stim,function=lambda x: [0]*dim) #context-extinction circuit
